...
wp_register_style('Gallery-Circle', plugins_url( 'css/style-gallery.css', __FILE__ ), array(), '', 'all' );
...
function shortcode_showGallery( $atts ) {
	...
	if($post_thumbnail==='false' || $image=='') {
		echo '<div class="circleImg"><img src="'.plugins_url( 'images/'.$default_image, __FILE__ ).'" title="Read more about '.$title.'" alt="'.$title.'"/></div>';
	}
	else{
	...
	$firstPost=true;
		while ( have_posts() ) : the_post();
			if($contPost>$post) {
				...
				if($post_thumbnail==='false' || $image=='') {
					echo '<img src="'.plugins_url( 'images/'.$default_image, __FILE__ ).'" title="Read more about '.$title.'" alt="'.$title.'"/>';
				}
				else{
